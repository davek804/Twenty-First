...
public class RegisterTask extends AsyncTask<String, Void, Integer> {
	 ...
	 * */
	public RegisterTask(Polling activity, ProgressDialog progressDialog) {
	...
	protected Integer doInBackground(String... arg0) {
        ...
        JSONObject json = userFunction.registerUser(userName.getText().toString(), passwordEdit.getText().toString());
	...
	protected void onPostExecute(Integer responseCode)
	{
		...
		if (responseCode == 1) {
			...
			Toast toast = Toast.makeText(context, "Successfully registered " +
					s, duration);
			toast.show();
			userName.setText("");
		...
		if (responseCode == 0) {
			progressDialog.dismiss();
			int duration = Toast.LENGTH_SHORT;
			Toast toast = Toast.makeText(context, "Registration failed", duration);
			toast.show();
		}
